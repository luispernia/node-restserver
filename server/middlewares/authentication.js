...
let verifyToken = (req, res, next) => { // Midleware
    let token = req.get("Authorization") // req.get() for get headers params
    jwt.verify(token, process.env.TOKEN_SEED, (err, decoded) => { // Verify the token, token, seed, decoded
        if (err) {
...
let verifyRole = (req, res, next) => { // Midleware
